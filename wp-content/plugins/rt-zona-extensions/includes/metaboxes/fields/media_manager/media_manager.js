...
jQuery.fn.MediaManager = function( options ) {
	...
	return this.each( function( i ) {		  
		...
		function _mm_editor() {
			...
			$.ajax({
				...
				success: function( response ) {
					...
					/* -----------------  Add hearthis track */
					$( '.add-hearthis', edit_box ).on( 'click', function() {

						var 
							url = $( '#mm-audio-custom-url', edit_box ).val(),
							api_url = 'https://api-v2.hearthis.at',
							fragment,
							fragment_two,
							finall_url;

							$( '.services-messages .msg', edit_box ).hide();
							...
							if ( url.match(/hearthis.at/) && ! url.match(/listen/) ) {
								...
								$.ajax({
									...
									if ( typeof response =='object' ){
										...
										if ( response.stream_url ) {
											...
											// Fill data
											// URL
											$( '#mm-audio-custom-url', edit_box ).val( response.stream_url );
											// Title
											$( '#mm-audio-title', edit_box ).val( response.title );
											// Artists
											$( '#mm-track_artists', edit_box ).val( response.user.username );
											// Artists URI
											$( '#mm-artists_url', edit_box ).val( response.user.permalink_url );
											$( '#mm-artists_target', edit_box ).val( '_blank' ).trigger('change');
											// Cover
											$( '.cover-source', edit_box ).val( 'external_link' ).trigger('change');
											$( '#r-cover', edit_box ).val( response.artwork_url );
											// Release
											$( '#mm-release_url', edit_box ).val( response.permalink_url );
											$( '#mm-release_target', edit_box ).val( '_blank' ).trigger('change');
											// Downlable
											if ( response.downloadable == 1 ) {
												$( '#mm-free_download', edit_box ).val( 'yes' ).trigger('change');
												$( '#mm-cart_url', edit_box ).val( response.download_url );
											}
											$( '.services-messages .msg-done', edit_box ).show();

										} else {
											// Stream does'n esists
											$( '.services-messages .msg-track-error', edit_box ).show();
										}
								...
								if ( url.indexOf( "/listen/" ) != 0 ) {
									$( '.services-messages .msg-already-exists', edit_box ).show();
								} else {	
									$( '.services-messages .msg-correct-link', edit_box ).show();
								}
							}
					...
					$( '.add-googledrive', edit_box ).on( 'click', function() {
						...
						var 
							...
							if ( url.indexOf('drive.google.com') >= 0 && url.indexOf('open?id=') >= 0 && url.indexOf('uc?export=download') < 0 ) {
								...
								if ( file_id != '' ) {
									...
									$( '#mm-audio-custom-url', edit_box ).val( finall_url  );
					...
					$( '#mm-editor-content', edit_box ).show();
		...
		function _mm_editor_save() {
			...
			} else if ( $( '#mm-editor-content .track-cover' ).attr( 'type' ) == 'text' && $( '#mm-editor-content .track-cover' ).val() != '' ) {
				$audio_cover = $( '#mm-editor-content .track-cover' ).val();
				$( '#'+item_id, mm ).find( '.mm-item-preview img' ).addClass('mm-audio-custom-cover').attr( 'src', $audio_cover );
